...
get_tunnel_database_url() {
  ...
  eval `ssh-agent -s` && ssh-add /config/ssh_keys/scalingo
  ...
  then
    ...
    /usr/local/bin/scalingo --region $SCALINGO_REGION -a "$app_name" db-tunnel -i /config/ssh_keys/scalingo postgres://"$POSTGRESQL_URL" &
    sleep 3
    DB_TUNNEL_PID=$!
  fi
